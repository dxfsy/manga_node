...
const { getIndexList, getDetailPageList, getComic, getSearchList, getLabelList, getLabels, getChapterList } = require('../spider/request')
...
router.post('/chapter', async (req, res, next) => {
    let { comicId } = req.body
    let list = await getChapterList(comicId)
    console.log('/comic/chapter post');
    res.json(new SuccessModel(list))
})
